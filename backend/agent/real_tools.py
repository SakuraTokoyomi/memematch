...
project_root = os.path.dirname(backend_dir)  # memematch/

# 定义模块路径（确保在全局作用域）
generate_module_path = os.path.join(project_root, 'member_c_generate')
generator_backend_path = os.path.join(backend_dir, 'generator')
...
try:
    # 尝试新结构
    from backend.search.engine import search_meme as search_meme_real
...
try:
    # 尝试新结构
    from backend.generator.generate_meme import generate_meme as generate_meme_real
    REAL_GENERATE_AVAILABLE = True
    GENERATE_SOURCE = "new"  # 标记使用新结构
    print("✅ Meme生成器已成功导入（新结构）")
except ImportError:
    # 兼容旧结构
    try:
        ...
        REAL_GENERATE_AVAILABLE = True
        GENERATE_SOURCE = "old"  # 标记使用旧结构
        print("✅ Meme生成器已成功导入（旧结构）")
    except ImportError as e:
        print(f"⚠️  无法导入Meme生成器: {e}")
...
def real_generate_meme(text: str, template: str = "drake", options: Dict = None, **kwargs) -> Dict[str, Any]:
    ...
    try:
        ...
        # 根据导入来源决定工作目录
        if GENERATE_SOURCE == "new":
            # 新结构：backend/generator/
            work_dir = generator_backend_path
        else:
            # 旧结构：member_c_generate/
            work_dir = generate_module_path
        
        # 切换到生成器目录（因为生成器依赖相对路径）
        os.chdir(work_dir)
        logger.debug(f"📂 [real_generate_meme] 工作目录: {work_dir}")
        ...
        try:
            ...
            if result.get("success"):
                ...
                # 转换为相对于项目根目录的路径
                if GENERATE_SOURCE == "new":
                    # backend/generator/outputs/...
                    project_relative_path = os.path.join("backend", "generator", relative_path)
                else:
