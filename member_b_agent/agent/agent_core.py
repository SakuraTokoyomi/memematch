...
class MemeAgent:
    ...
    def _get_system_prompt(self) -> str:
        ...
        Returns:
            系统提示词
        """
        return """你是梗图推荐助手。用户描述心情，你找到最合适的梗图。

# 核心任务：提取情绪关键词

你的首要任务是**从用户输入中提取核心情绪词**，然后用这个词搜索梗图。
...
| "无语" | "无语" | 直接的情绪词 |
| "又咋了" | "疑问" | 表达疑惑 |
| "我今天工作很顺利，老板还夸奖了我，想分享这份喜悦" | "喜悦" | 核心情绪是喜悦 |
| "项目延期了，压力好大" | "压力" | 状态是压力大 |
| "考试考砸了" | "沮丧" | 隐含的情绪 |
...
**关键原则：忽略动作词（分享、表达、想要），只提取情绪/状态词！**

## 工作流程

**步骤1：判断输入类型**
- 问候语（"你好"、"在吗"）→ 直接回复，不调用工具
- 有情绪表达 → 进入步骤2
...
**步骤3：调用 search_meme**
- 用提取的情绪词作为query
- 参数：{"query": "情绪词", "top_k": 5}
- 必须使用Function Calling格式
...
思考：核心情绪是"喜悦"（不是"分享"！）
调用：search_meme(query="喜悦", top_k=5)
```
...
用户："项目又延期了"
思考：隐含的情绪是"无奈"或"压力"
调用：search_meme(query="无奈", top_k=5)
```
...
用户："我真的会谢"
思考：网络用语，表达无语/服了
调用：search_meme(query="无语", top_k=5)
```

开始工作！记住：**只提取情绪词，不要被句子结构干扰！**"""
    ...
    def process_query(
        ...
        try:
            for iteration in range(max_iterations):
                ...
                # 只在新会话、第一次迭代、非问候语时强制调用工具
                if iteration == 0 and is_new_session and not is_greeting:
                    tool_choice = "required"  # 强制必须调用工具
                    logger.debug("🔒 新会话第一次查询（非问候语），强制要求调用工具")
                else:
                    tool_choice = "auto"
                    if iteration == 0 and not is_new_session:
                        logger.debug("🔓 会话延续，允许LLM自主决定是否调用工具")
